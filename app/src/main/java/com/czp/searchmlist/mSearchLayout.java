...
public class mSearchLayout extends LinearLayout{
    ...
    private void  InitView(){
        ...
        //把获得的view加载到这个控件中
        addView(searchview);
        scrollView=searchview.findViewById(R.id.scrollView);
    ...
    private class MyTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (et_searchtext_search.getText().length()==0)
                sCBlistener.OnCanclePress();
    ...
    private void setLinstener() {
        ...
        iv_back.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (et_searchtext_search.getText().length()!=0)
                    ib_searchtext_delete.callOnClick();
                else
                    sCBlistener.Back();
            }
        });
...
//                    Toast.makeText(context, "搜索" +searchtext, Toast.LENGTH_SHORT).show();
                    ...
                    et_searchtext_search.clearFocus();
                    return true;
                }
                return false;
            }
...
//                Toast.makeText(context, "Item点击"+string, Toast.LENGTH_SHORT).show();
                ...
                et_searchtext_search.setText(string);
                iv_back.setFocusableInTouchMode(true);
                et_searchtext_search.clearFocus();
            }
        };
...
//                Toast.makeText(context, "历史记录点击"+OldDataList.get(position), Toast.LENGTH_SHORT).show();
                if(sCBlistener!=null){
                    et_searchtext_search.setText(OldDataList.get(position).trim());
