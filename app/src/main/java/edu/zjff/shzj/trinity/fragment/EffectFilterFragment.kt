...
import edu.zjff.shzj.trinity.adapter.EffectAdapter
...
class EffectFilterFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    stream.read(buffer)
    stream.close()
    val effects = Gson().fromJson<List<Effect>>(String(buffer, Charset.forName("UTF-8")), object: TypeToken<List<edu.zjff.shzj.trinity.entity.Effect>>(){}.type)
